...
Tree.prototype = {
    ...
    update: function update(value, $el) {
        $el.data(value);
        // add folder icon class
        $el.find(this.classes.label).html(value.title);
    },

    append: function append(value, $parent, $pparent) {
        var _this = this;
        var $entity;
        ...
        // add child nodes
        if ($parent.hasClass(_this.classes.branchHeader)) {
            $pparent.find('.' + _this.classes.branchChildren).eq(0).append($entity);
        } else {
